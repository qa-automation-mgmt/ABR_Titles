...
public class FilterTableTest extends BaseTest {
    ...
    public void testReactSelectFiltersSequentially() throws InterruptedException {
        ...
        System.out.println("🔹 Starting filter + export test on Titles page...");
    ...
    private void applyAndVerifyFilters(WebDriverWait wait, JavascriptExecutor js, Actions actions,
        ...
        for (int attempt = 1; attempt <= 3 && !opened; attempt++) {
            try {
                ...
                System.out.println("⚠️ Attempt " + attempt + " failed to open dropdown for " + column);
            }
        }

        Assert.assertTrue(opened, "❌ Failed to open dropdown for " + column);
        ...
        for (int i = 0; i < numFilters; i++) {
            ...
            if (multiple && i < numFilters - 1) {
                try {
                    WebElement reopen = input != null ? input : selectControl;
        ...
        System.out.println("✅ Applied " + (multiple ? "multiple" : "single") +
                " random filter(s) for " + column + ": " + selectedValues);
